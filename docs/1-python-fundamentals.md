...
容易看出，我们给变量a先后赋值了整数类型，字符串类型和浮点数。既然变量的类型是随时有可能变的，那么我们怎么知道一个变量的具体类型呢？可以利用type函数：
...
如果你学过C/C++语言，那么这可能是你第一次听说“**垃圾回收**”这个词。垃圾回收是现代编程语言中非常重要的技术，Java，Python，Javascript等编程语言都有垃圾回收机制。垃圾回收，**垃圾**指的是那些程序不再需要的内存对象，**回收**就是系统会自动把这些内存删除掉。因此，在具有垃圾回收的语言里你不用再担心内存泄漏啦！
...
字符串格式化虽然简单，但却是非常常用的一个功能。在Python里面有以下几种方法：
...
set、list、tuple、dict都是容器类型，元组tuple就是不可修改的列表list，所以基本的数据结构只有集合，列表，字典。与set，list，dict相对应，C++的标准库里也支持list，set，map等类型，但远没有在Python里使用起来方便。
